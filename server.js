...
class SmartEVBatteryServer {
    ...
    setupBatteryRoutes() {
        ...
        this.app.post('/api/battery/predict', async (req, res) => {
            try {
                const response = await axios.post(`${this.pythonApiUrl}/predict/battery`, req.body);
                res.json({
                    success: true,
                    data: response.data,
                    timestamp: new Date().toISOString()
        ...
        this.app.post('/api/battery/soh', async (req, res) => {
            try {
                const response = await axios.post(`${this.pythonApiUrl}/predict/soh`, req.body);
                res.json({
                    success: true,
                    data: response.data,
                    timestamp: new Date().toISOString()
        ...
        this.app.post('/api/battery/rul', async (req, res) => {
            try {
                const response = await axios.post(`${this.pythonApiUrl}/predict/rul`, req.body);
                res.json({
                    success: true,
                    data: response.data,
                    timestamp: new Date().toISOString()
        ...
        this.app.post('/api/battery/analyze', async (req, res) => {
            try {
                ...
                const response = await axios.post(`${this.pythonApiUrl}/predict/battery`, batteryData);
        ...
        this.app.post('/api/battery/simulate', async (req, res) => {
            try {
                const response = await axios.post(`${this.pythonApiUrl}/simulate/trip`, req.body);
                res.json({
                    success: true,
                    data: response.data,
                    timestamp: new Date().toISOString()
        ...
        this.app.get('/api/dashboard/data', async (req, res) => {
            try {
                ...
                const dashboardData = {
                    ...
                    recent_alerts: [
                        { id: 1, type: 'warning', message: 'Battery #45 temperature elevated', timestamp: new Date().toISOString() },
                        { id: 2, type: 'info', message: 'Charging cycle completed for Battery #23', timestamp: new Date().toISOString() }
                    ]
                };
    ...
    generateAlerts(analysis) {
        ...
        if (soh < 70) {
            alerts.push({ level: 'critical', message: 'Battery health critically low' });
        } else if (soh < 80) {
            alerts.push({ level: 'warning', message: 'Battery health declining' });
        }
        
        if (temp > 40) {
            alerts.push({ level: 'critical', message: 'Battery temperature too high' });
        } else if (temp > 35) {
            alerts.push({ level: 'warning', message: 'Battery temperature elevated' });
        }
    ...
    async startPythonAPI() {
        ...
        this.pythonProcess.stderr.on('data', (data) => {
            ...
            if (!message.includes('WARNING') && !message.includes('InconsistentVersionWarning')) {
    ...
    async waitForPythonAPI(maxAttempts = 30, interval = 1000) {
        for (let i = 0; i < maxAttempts; i++) {
            try {
                const response = await axios.get(`${this.pythonApiUrl}/health`, { timeout: 2000 });
    ...
    start() {
        this.app.listen(this.port, () => {
            ...
            console.log(`🔋 Battery API: http://localhost:${this.port}/api/battery/predict`);
            console.log('==========================================');
        });
