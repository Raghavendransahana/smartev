...
class SmartEVBatteryServer {
    ...
    setupRoutes() {
        ...
        this.app.post('/battery/update', (req, res) => {
            ...
            if (temperature) this.currentBatteryData.temperature = parseFloat(temperature);
        ...
        this.app.get('/', (req, res) => {
            res.json({
                ...
                sample_usage: {
                    ...
                    update_battery: 'curl -X POST http://localhost:8000/battery/update -H "Content-Type: application/json" -d \'{"voltage": 3.8, "soc": 90}\''
                }
            });
        });
        ...
        this.app.use('*', (req, res) => {
            res.status(404).json({
                ...
                available_endpoints: ['/battery', '/battery/soh', '/battery/rul', '/battery/metrics', '/health']
            });
        });
    }
    ...
    simulateRealTimeData() {
        ...
        this.currentBatteryData.temperature = 20 + Math.random() * 15 + (loadFactor - 1) * 5;
        
        // SOC decreases during high load, increases during charging periods
        if (Math.random() > 0.7) { // 30% chance of charging
            this.currentBatteryData.soc = Math.min(100, this.currentBatteryData.soc + Math.random() * 2);
        } else {
            this.currentBatteryData.soc = Math.max(20, this.currentBatteryData.soc - Math.random() * 0.5 * loadFactor);
        }
    ...
    runMLPrediction() {
        ...
        if (!fs.existsSync(venvPython)) {
            console.log('⚠️  Virtual environment not found, using fallback calculations');
            this.calculateFallbackPredictions();
            return;
        }
        
        // Run Python prediction using virtual environment
        const pythonProcess = spawn(venvPython, [pythonScript, JSON.stringify(inputData)], {
            cwd: mlDir,
    ...
    updateMLResults(prediction) {
        ...
        this.currentBatteryData.health_status = this.getHealthStatus(this.currentBatteryData.soh);
