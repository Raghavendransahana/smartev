...
function updateBatteryData() {
    ...
    if (Math.random() > 0.7) { // 30% chance charging
        batteryData.soc = Math.min(100, batteryData.soc + Math.random() * 2);
    } else {
        batteryData.soc = Math.max(20, batteryData.soc - Math.random() * 0.5 * loadFactor);
    }
    ...
    console.log(`🔄 Update #${batteryData.update_count}: SOH=${batteryData.soh}%, RUL=${batteryData.rul} cycles, SOC=${batteryData.soc.toFixed(1)}%`);
}
...
function useFallbackCalculation() {
    ...
    let soh = 100 - ((Math.abs(temperature - 25)) * 0.2) - ((Math.abs(voltage - 3.7)) * 8);
